...
class AnalyzerControllerTest {
	...
	private static final double DEVIATION_CREATER_MAX = VALUE_GREATER_MAX-MAX_VALUE;
	private static final double DEVIATION_LESS_MIN = VALUE_LESS_MIN-MIN_VALUE;
	private static final ProbeData probeNormalData = new ProbeData(SENSOR_ID, NORMAL_VALUE, System.currentTimeMillis());
	private static final ProbeData probeGreaterMaxData = new ProbeData(SENSOR_ID, VALUE_GREATER_MAX, System.currentTimeMillis());
	private static final ProbeData probeLessMinData = new ProbeData(SENSOR_ID, VALUE_LESS_MIN, System.currentTimeMillis());
	...
	void testGreaterMaxDeviation() throws Exception {
		...
		assertEquals(DEVIATION_CREATER_MAX, deviation.devitation());
	...
	void testLessMinDeviation() throws Exception {
		...
		DeviationData deviation = mapper.readValue(message.getPayload(), DeviationData.class);
		assertEquals(SENSOR_ID, deviation.id());
		assertEquals(DEVIATION_LESS_MIN, deviation.devitation());
